  ...
  return <>
    ...
    <main className="relative flex-1 p-6 pt-28">
      ...
      <header className='flex flex-col gap-3 md:flex-row md:justify-between md:items-center'>
        <h2 className='text-xl text-orange-500 font-medium md:text-2xl md:font-semibold'>{user && user.role === "Admin" ? "Book Management" : "Books"}</h2>
        <div className="flex flex-col lg:flex-row space-y-4 lg:space-y-0 lg:space-x-4">
          {
            isAuthenticated && user.role === "Admin" && (
              <button onClick={() =>  dispatch(toggleAddBookPopup())} className='w-full pl-14 relative sm:w-52 flex gap-4 items-center justify-center py-2 px-4 bg-orange-500 text-white rounded-md hover:bg-orange-600 font-bold'> <span className='bg-white flex justify-center items-center overflow-hidden rounded-full text-black w-[25px] h-[25px]  text-[27px] absolute left-5'>+</span>Add Book</button>
            )
          }
          <input type="text" className='w-full sm:w-52 border p-2 border-orange-500 rounded-md' placeholder='Search Books...' value={searchedKeyword} onChange={handleSearch} />
        </div>
      </header>
              ...
              <tbody>

                {
                  searchedBooks.map((book, index) => (
                    <tr key={book._id} className={(index + 1) % 2 === 0 ? "bg-gray-50" : "bg-orange-100"}>
                      ...
                      <td className='px-4 py-2'>{book.availability ? "Available" : "Unavailable"}</td>
                      {
                        isAuthenticated && user.role === "Admin" && (
                          <td className='px-4 py-2 flex space-x-2 my-3 justify-center'>
                            <FaBookOpen onClick={()=>openReadPopup(book._id)}/>
                              <FaPen onClick={()=>openRecordBookPopup(book._id)}/>
                          </td>
                        )
                      }
                    </tr>
