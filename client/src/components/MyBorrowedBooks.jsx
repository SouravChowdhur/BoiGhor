  ...
  return <>
    <main className="relative flex-1 p-6 pt-28">
      ...
      <header className='flex flex-col gap-3 sm:flex-row md:items-center'>
        <button className={`relative rounded sm:rounded-tr-none sm:rounded-br-none sm:rounded-bl-lg sm:rounded-tl-lg text-center font-semibold py-2 border-2 w-full sm:w-72 ${filter === "returned" ? "bg-orange-500 text-white border-orange-500" : "border-gray-200 bg-gray-200 text-black hover:bg-gray-300"}`}
          onClick={() => setFilter("returned")}>
          Returned Books
        </button>
        <button className={`relative rounded sm:rounded-tl-none sm:rounded-bl-none sm:rounded-tr-lg sm:rounded-br-lg text-center font-semibold py-2 border-2 w-full sm:w-72 ${filter === "nonReturned" ? "bg-orange-500 text-white border-orange-500" : "border-gray-200 bg-gray-200 text-black hover:bg-gray-300"}`}
...

      {
              ...
              <tbody>
                {
                  booksToDisplay.map((book, index) => (
                    <tr key={index} className={(index + 1) % 2 === 0 ? "bg-gray-50" : "bg-orange-100"}>
                      ...
                      <td className='px-4 py-2'><FaBookOpen onClick={()=>openReadPopup(book.bookId)}/></td>
                    </tr>
                  ))
                }
              </tbody>
        ...
        ) : filter === "returned" ? (<h3 className='text-3xl mt-5 font-medium'>No Returned Books Found!!</h3>) : (<h3 className='text-3xl mt-5 font-medium'>No Non-Returned Books Found!!</h3>)
      }
    </main>
